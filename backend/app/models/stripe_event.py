...
class StripeEvent(Base):
    ...
    event_id = Column(String(255), unique=True, nullable=False, index=True)  # Alias for stripe_event_id
    event_type = Column(String(100), nullable=False, index=True)
    processed = Column(Boolean, default=False, nullable=False)
    processed_at = Column(DateTime(timezone=True), nullable=True)  # When the event was processed
